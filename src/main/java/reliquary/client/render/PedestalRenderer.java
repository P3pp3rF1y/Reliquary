...
public class PedestalRenderer implements BlockEntityRenderer<PedestalBlockEntity> {
	@Override
	public void render(PedestalBlockEntity blockEntity, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay,
			Vec3 cameraPos) {
		if (!blockEntity.getItem().isEmpty()) {
			...
			Minecraft.getInstance().getItemRenderer().renderStatic(stack, ItemDisplayContext.GROUND, packedLight, OverlayTexture.NO_OVERLAY, poseStack, buffer,
					null, 0);
			poseStack.popPose();
			PedestalClientRegistry.getItemRenderer(stack)
					.ifPresent(extraRenderer -> extraRenderer.doRender(blockEntity, partialTicks, poseStack, buffer, packedLight, packedOverlay));
		}
	}
