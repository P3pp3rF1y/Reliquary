...
public class FortuneCoinItem extends ItemBase implements IPedestalActionItem, ICuriosItem {
	...
	private void teleportEntityToPlayer(Entity item, Player player) {
		player.level().addParticle(ParticleTypes.ENTITY_EFFECT, item.getX() + 0.5D + player.level().random.nextGaussian() / 8, item.getY() + 0.2D,
				item.getZ() + 0.5D + player.level().random.nextGaussian() / 8, 0.9D, 0.9D, 0.0D);
		player.getLookAngle();
			...
			if (amountAdded > 0) {
				...
				if (amountToTransfer > amountAdded) {
					world.addFreshEntity(
							new ExperienceOrb(world, pos.getX(), pos.getY(), pos.getZ(), XpHelper.liquidToExperience(amountToTransfer - amountAdded)));
				}
			} else {
				pedestal.setActionCoolDown(20);
			}
