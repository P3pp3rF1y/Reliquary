...
public class PyromancerStaffItem extends ChargeableItem implements IScrollableItem {
	...
	private void doEruptionAuxEffects(Player player, int soundX, int soundY, int soundZ) {
		if (player.level().getRandom().nextFloat() < 0.1F) {
			player.level().playLocalSound(soundX + 0.5D, soundY + 0.5D, soundZ + 0.5D, SoundEvents.FIRECHARGE_USE, SoundSource.NEUTRAL, 0.2F,
					0.03F + (0.07F * player.level().getRandom().nextFloat()), false);
		}
	...
	private void spawnFlameParticles(Player player, int soundX, int soundY, int soundZ) {
		for (int particleCount = 0; particleCount < 6; ++particleCount) {
			...
			if (Math.abs(randX - (soundX + 0.5D)) < 4.0D && Math.abs(randZ - (soundZ + 0.5D)) < 4.0D) {
				player.level().addParticle(ParticleTypes.FLAME, randX, soundY + 1D, randZ, player.level().getRandom().nextGaussian() * 0.2D,
						player.level().getRandom().nextGaussian() * 0.2D, player.level().getRandom().nextGaussian() * 0.2D);
			}
		}
		...
		BlockPos.betweenClosed(player.blockPosition().offset(-3, -3, -3), player.blockPosition().offset(3, 3, 3)).forEach(pos -> {
			...
			if (block instanceof BaseFireBlock) {
				...
				player.level().playSound(null, pos, SoundEvents.LAVA_EXTINGUISH, SoundSource.BLOCKS, 0.5F,
						2.6F + RandHelper.getRandomMinusOneToOne(player.level().getRandom()) * 0.8F);
			}
		});
	}
