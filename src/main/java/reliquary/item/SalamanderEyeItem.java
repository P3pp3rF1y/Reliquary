		...
		BlockPos.betweenClosed(player.blockPosition().offset(-3, -3, -3), player.blockPosition().offset(3, 3, 3))
				.forEach(pos -> {
					if (player.level().getBlockState(pos).getBlock() instanceof BaseFireBlock) {
						...
						player.level().playSound(null, pos, SoundEvents.FIRE_EXTINGUISH, SoundSource.NEUTRAL, 0.5F, 2.6F + RandHelper.getRandomMinusOneToOne(player.level().getRandom()) * 0.8F);
					}
				});
	}

	private void doFireballEffect(Player player) {
		List<Projectile> projectiles = player.level().getEntitiesOfClass(Projectile.class, player.getBoundingBox().inflate(5), projectile -> projectile.getType().builtInRegistryHolder().is(EntityTypeTags.REDIRECTABLE_PROJECTILE));
		for (Projectile projectile : projectiles) {
			...
			player.level().playLocalSound(projectile.getX(), projectile.getY(), projectile.getZ(), SoundEvents.FIRE_EXTINGUISH, SoundSource.NEUTRAL, 0.5F, 2.6F + RandHelper.getRandomMinusOneToOne(player.level().getRandom()) * 0.8F, false);
		}
		...
		for (SmallFireball fireball : blazeFireballs) {
			...
			player.level().playLocalSound(fireball.getX(), fireball.getY(), fireball.getZ(), SoundEvents.FIRE_EXTINGUISH, SoundSource.NEUTRAL, 0.5F, 2.6F + RandHelper.getRandomMinusOneToOne(player.level().getRandom()) * 0.8F, false);
			fireball.discard();
		}
	}
}
