...
public class SalamanderEyeItem extends ItemBase {
	...
	private void doFireballEffect(Player player) {
		List<Projectile> projectiles = player.level().getEntitiesOfClass(Projectile.class, player.getBoundingBox().inflate(5),
				projectile -> projectile.getType().builtInRegistryHolder().is(EntityTypeTags.REDIRECTABLE_PROJECTILE));
