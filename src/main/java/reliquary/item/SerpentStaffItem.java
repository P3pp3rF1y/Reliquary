...
public class SerpentStaffItem extends ItemBase {
	...
	private void shootKrakenSlime(ItemStack serpentStaff, Player player) {
		player.level().playSound(null, player.blockPosition(), SoundEvents.ARROW_SHOOT, SoundSource.NEUTRAL, 0.5F, 0.4F / (player.level().getRandom().nextFloat() * 0.4F + 0.8F));
	...
	public boolean onLeftClickEntity(ItemStack stack, Player player, Entity entity) {
		...
		//drain effect
		int drain = player.level().getRandom().nextInt(4);
