...
public class AlkahestryCraftingRecipeCategory extends AlkahestryRecipeCategory<AlkahestryCraftingRecipe> {
	...
	@Override
	public void draw(AlkahestryCraftingRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphicsExtractor guiGraphics, double mouseX, double mouseY) {
		background.draw(guiGraphics);
		String chargeString = "-" + recipe.getChargeNeeded();
		Font fontRenderer = Minecraft.getInstance().font;
		int stringWidth = fontRenderer.width(chargeString);
		guiGraphics.text(fontRenderer, chargeString, (int) (((double) background.getWidth() - stringWidth) / 2), 40, -8355712);
	}
}
