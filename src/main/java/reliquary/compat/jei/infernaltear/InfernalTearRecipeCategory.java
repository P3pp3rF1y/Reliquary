...
public class InfernalTearRecipeCategory extends AbstractRecipeCategory<InfernalTearRecipe> {
	...
	private static final Identifier BACKGROUNDS_TEXTURE = Reliquary.getIdentifier("textures/gui/jei/backgrounds.png");
	...
	public void draw(InfernalTearRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
		...
		guiGraphics.drawString(fontRenderer, points, (int) ((double) background.getWidth() / 2 + (((double) background.getWidth() / 2 + 16 - stringWidth) / 2)),
				5, XP_COLOR);
