...
import reliquary.blocks.tile.ApothecaryMortarTileEntity;
...
public class DataProviderMortar extends CachedBodyDataProvider {
	...
	public List<String> getWailaBodyToCache(ItemStack itemStack, IWailaDataAccessor accessor, IWailaConfigHandler config) {
		...
		if(!effects.isEmpty()) {
			currenttip.add(ChatFormatting.WHITE + LanguageHelper.getLocalization("waila.reliquary.mortar.result") + ChatFormatting.RESET);
