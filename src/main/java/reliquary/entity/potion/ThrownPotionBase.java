...
public abstract class ThrownPotionBase extends ThrowableItemProjectile {
	...
	<T extends ThrownPotionBase> ThrownPotionBase(EntityType<T> entityType, Level level, Player player, ItemStack thrownStack) {
		super(entityType, player, level, thrownStack);
	}
	...
	<T extends ThrownPotionBase> ThrownPotionBase(EntityType<T> entityType, Level level, double x, double y, double z, ItemStack thrownStack) {
		super(entityType, x, y, z, level, thrownStack);
	}
	...
	private void spawnParticles() {
		...
		for (int i = 0; i < 8; ++i) {
			level().addParticle(new ItemParticleOption(ParticleTypes.ITEM, ItemStackTemplate.fromNonEmptyStack(getItem())), getX(), getY(), getZ(),
	...
	protected Item getDefaultItem() {
		return ModItems.FERTILE_POTION.get();
	}
}
