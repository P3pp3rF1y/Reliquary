...
public class AphroditePotionEntity extends ThrownPotionEntity {
	...
	void doLivingSplashEffect(LivingEntity el) {
		...
		for (int var3 = 0; var3 < 7; ++var3) {
			...
			level().addParticle(ParticleTypes.HEART, animal.getX() + random.nextFloat() * animal.getBbWidth() * 2.0F - animal.getBbWidth(),
					animal.getY() + 0.5D + random.nextFloat() * animal.getBbHeight(),
					animal.getZ() + random.nextFloat() * animal.getBbWidth() * 2.0F - animal.getBbWidth(), var4, var6, var8);
		}
	}
