...
public class KrakenSlime extends ThrowableItemProjectile {
	...
	public KrakenSlime(Level level, Player player) {
		super(ModEntities.KRAKEN_SLIME.get(), player.getX(), player.getEyeY() - 0.1, player.getZ(), level, new ItemStack(Items.SLIME_BALL));
		setOwner(player);
	}
	...
	protected void onHit(HitResult result) {
		...
		for (int count = 0; count < 6; ++count) {
			...
			level().addParticle(ParticleTypes.ITEM_SLIME, getX() - motionX * amplifier + level().random.nextDouble(),
					getY() - motionY * amplifier + level().random.nextDouble(), getZ() - motionZ * amplifier + level().random.nextDouble(), motionX, motionY,
					motionZ);
		}
