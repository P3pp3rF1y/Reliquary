...
public class ReliquaryModelProvider extends ModelProvider {
	...
	private static final ModelTemplate PEDESTAL_TEMPLATE = ModelTemplates.create(Reliquary.getIdentifier("pedestal").toString(), WOOL_SLOT);
	private static final ModelTemplate PASSIVE_PEDESTAL_TEMPLATE = ModelTemplates.create(Reliquary.getIdentifier("passive_pedestal").toString(), WOOL_SLOT);
	...
	protected void registerModels(BlockModelGenerators blockModels, ItemModelGenerators itemModels) {
		...
		generateCustomModel(itemModels, ModItems.HANDGUN.get(), ItemModelUtils.plainModel(Reliquary.getIdentifier("handgun").withPrefix("item/")));
	...
	private void generateAlkahestryAltar(BlockModelGenerators blockModels) {
		...
		Identifier activeBlockModelId = ModelTemplates.CUBE_ALL.create(activeAlkahestryAltar, TextureMapping.cube(new Material(activeAlkahestryAltar)),
	...
	private void generateCharmAndFragment(ItemModelGenerators itemModels) {
		...
		Identifier modelLocation = ModelLocationUtils.getModelLocation(charmItem);
	...
	private static void generatePassivePedestal(BlockModelGenerators blockModels, Block pedestal, DyeColor color) {
		Identifier blockModelId = PASSIVE_PEDESTAL_TEMPLATE.create(pedestal,
				new TextureMapping().put(WOOL_SLOT, new Material(Identifier.parse(color.getName() + "_wool").withPrefix("block/"))), blockModels.modelOutput);
	...
	private static void generatePedestal(BlockModelGenerators blockModels, Block pedestal, DyeColor color) {
		Identifier blockModelId = PEDESTAL_TEMPLATE.create(pedestal,
				new TextureMapping().put(WOOL_SLOT, new Material(Identifier.parse(color.getName() + "_wool").withPrefix("block/"))), blockModels.modelOutput);
	...
	private static void generateFertileLilypad(BlockModelGenerators blockModels) {
		...
		Identifier itemModelId = blockModels.createFlatItemModelWithBlockTexture(block.asItem(), block);
