...
public class ReliquaryModelProvider extends ModelProvider {
	...
	private void generateAlkahestryAltar(BlockModelGenerators blockModels) {
		...
		Identifier activeBlockModelId = ModelTemplates.CUBE_ALL.create(activeAlkahestryAltar, TextureMapping.cube(activeAlkahestryAltar),
	...
	private static void generatePassivePedestal(BlockModelGenerators blockModels, Block pedestal, DyeColor color) {
		Identifier blockModelId = PASSIVE_PEDESTAL_TEMPLATE.create(pedestal,
				new TextureMapping().put(WOOL_SLOT, Identifier.parse(color.getName() + "_wool").withPrefix("block/")), blockModels.modelOutput);
