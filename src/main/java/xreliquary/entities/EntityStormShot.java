...
public class EntityStormShot extends Entity implements IProjectile {
    ...
    public EntityStormShot(World par1World, double par2, double par4,
            double par6) {
        super(par1World);
        this.setSize(0.5F, 0.5F);
    ...
    private void spawnHitParticles(String string, int i) {
        for (int particles = 0; particles < i; particles++) {
            worldObj.spawnParticle(string, posX, posY
                    ...
                    rand.nextFloat() + motionY, gaussian(motionZ));
        }
    }

    private void doDamage(int i, Entity mop) {
        if (mop instanceof EntityCreeper) {
            ((EntityCreeper) mop).onStruckByLightning(new EntityLightningBolt(
                    worldObj, mop.posX, mop.posY, mop.posZ));
        } else {
            i *= 1 + worldObj.rainingStrength + worldObj.thunderingStrength * 2;
            if (worldObj.canLightningStrikeAt((int) (mop.posX + 0.5F),
                    (int) (mop.posY + 0.5F), (int) (mop.posZ + 0.5F))) {
                worldObj.addWeatherEffect(new EntityLightningBolt(worldObj,
                        mop.posX, mop.posY, mop.posZ));
            }
        }
