...
public class PassivePedestalRenderer extends TileEntityRenderer<PedestalPassiveTileEntity> {
	@Override
	public void render(PedestalPassiveTileEntity te, double x, double y, double z, float partialTicks, int destroyStage) {
		if (!te.getStackInSlot(0).isEmpty()) {
			ItemStack stack = te.getStackInSlot(0);
			GlStateManager.pushMatrix();
			float yDiff = MathHelper.sin((System.currentTimeMillis() % 86400000) / 1000F) * 0.1F + 0.1F;
			GlStateManager.translatef((float) x + 0.5F, (float) y + 0.65F + yDiff, (float) z + 0.5F);
