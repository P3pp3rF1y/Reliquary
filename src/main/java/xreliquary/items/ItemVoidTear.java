	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, @Nonnull EnumHand hand) {
		...
		if (!world.isRemote) {
			...
			if (rayTraceResult != null && rayTraceResult.typeOfHit == RayTraceResult.Type.BLOCK &&
					(InventoryHelper.hasItemHandler(world, rayTraceResult.getBlockPos()) && player.isSneaking() || getContainerItem(voidTear).getItem() instanceof ItemBlock))
		...
		for (int slot = 0; slot < player.inventory.getSizeInventory(); slot++) {
			...
			if (tearStack.getItem() == this && this.isEnabled(tearStack)) {
				...
				if (canAbsorbStack(pickedUpStack, tearStack)) {
					int playerItemQuantity = InventoryHelper.getItemQuantity(pickedUpStack, InventoryHelper.getItemHandlerFrom(player));
