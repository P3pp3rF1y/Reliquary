	...
	public static void addPotionInfo(PotionEssence essence, List list, boolean addEffectDescription) {
		if(essence.getEffects().size() > 0) {
			...
			if(!hashmultimap.isEmpty() && addEffectDescription) {
				list.add("");
				list.add(TextFormatting.DARK_PURPLE + StatCollector.translateToLocal("potion.effects.whenDrank"));
				...
				while(iterator1.hasNext()) {
					...
					if(d0 > 0.0D) {
						list.add(TextFormatting.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier2.getOperation(), new Object[] {ItemStack.DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String) entry1.getKey())}));
					} else if(d0 < 0.0D) {
						d1 *= -1.0D;
						list.add(TextFormatting.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier2.getOperation(), new Object[] {ItemStack.DECIMALFORMAT.format(d1), StatCollector.translateToLocal("attribute.name." + (String) entry1.getKey())}));
					}
				}
			}
		}
