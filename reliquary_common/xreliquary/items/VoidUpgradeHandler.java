...
public class VoidUpgradeHandler implements ICraftingHandler {
    ...
    public static Item[] validItems() {
        return new Item[] { XRItems.iceRod, XRItems.midasTouchstone,
                XRItems.voidSatchel, XRItems.sojournerStaff };
    }
    ...
    public static void upgradeCapacity(NBTTagCompound nbt, int upgrades) {
        for (; upgrades > 0; upgrades--) {
            if (!canUpgrade(nbt))
                return;
            setCapacity(nbt, getCapacity(nbt)
                    + Reference.CAPACITY_UPGRADE_INCREMENT);
        }
    }
    ...
    public void onCrafting(EntityPlayer player, ItemStack voidItem,
        ...
        for (int slot = 0; slot < craftMatrix.getSizeInventory(); slot++) {
            ...
            NBTTagCompound originalCapacityData = originalVoidItem
                    .getTagCompound();
            ...
            for (int slot0 = 0; slot0 < craftMatrix.getSizeInventory(); slot0++) {
                ...
                if (craftMatrix.getStackInSlot(slot0).getItem() == XRItems.emptyVoidTear) {
                    if (getCapacity(originalCapacityData)
                            + Reference.CAPACITY_UPGRADE_INCREMENT >= 32000) {
                    ...
                    if (getCapacity(originalCapacityData)
                            + Reference.CAPACITY_UPGRADE_INCREMENT * 4 >= 32000) {
                        craftMatrix.getStackInSlot(slot0).stackSize++;
                    } else {
