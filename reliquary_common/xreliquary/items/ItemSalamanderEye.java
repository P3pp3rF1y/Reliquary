...
public class ItemSalamanderEye extends ItemXR {
        ...
        for (int xOff = -3; xOff <= 3; xOff++) {
            for (int yOff = -3; yOff <= 3; yOff++) {
                for (int zOff = -3; zOff <= 3; zOff++)
                    if (player.worldObj
                            .getBlockId(x + xOff, y + yOff, z + zOff) == Block.fire.blockID) {
                        player.worldObj.setBlock(x + xOff, y + yOff, z + zOff,
                                0);
                        player.worldObj
                                .playSoundEffect(
                                        x + xOff + 0.5D,
    ...
    private void doFireballEffect(EntityPlayer player) {
        List ghastFireballs = player.worldObj.getEntitiesWithinAABB(
                EntityLargeFireball.class, AxisAlignedBB.getBoundingBox(
        ...
        while (fire1.hasNext()) {
            ...
            player.worldObj
                    .playSoundEffect(
                            ...
                            "random.fizz",
                            0.5F,
                            2.6F + (player.worldObj.rand.nextFloat() - player.worldObj.rand
                                    .nextFloat()) * 0.8F);
        }
        List blazeFireballs = player.worldObj.getEntitiesWithinAABB(
                EntitySmallFireball.class, AxisAlignedBB.getBoundingBox(
        ...
        while (fire2.hasNext()) {
            ...
            for (int particles = 0; particles < 4; particles++) {
                player.worldObj.spawnParticle("reddust", fireball.posX,
                        fireball.posY, fireball.posZ, 0.0D, 1.0D, 1.0D);
            }
            player.worldObj
                    .playSoundEffect(
                            fireball.posX,
