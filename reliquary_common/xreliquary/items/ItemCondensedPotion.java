...
public class ItemCondensedPotion extends ItemXR {
	...
	public EnumRarity getRarity(ItemStack stack) {
		if (hasEffect(stack)) return EnumRarity.epic;
	...
	public String getItemDisplayName(ItemStack ist) {
		...
		if (potion == PotionData.SPLASH_META) return Names.SPLASH_NAME;
		if (potion == PotionData.APHRODITE_META) return Names.APHRODITE_NAME;
		if (potion == PotionData.POISON_META) return Names.POISON_NAME;
		if (potion == PotionData.ACID_META) return Names.ACID_NAME;
		if (potion == PotionData.CONFUSION_META) return Names.CONFUSION_NAME;
		if (potion == PotionData.SLOWING_META) return Names.SLOWING_NAME;
		if (potion == PotionData.WEAKNESS_META) return Names.WEAKNESS_NAME;
		if (potion == PotionData.WITHER_META) return Names.WITHER_NAME;
		if (potion == PotionData.BLINDING_META) return Names.BLINDING_NAME;
		if (potion == PotionData.RUINATION_META) return Names.RUINATION_NAME;
		if (potion == PotionData.FERTILIZER_META) return Names.FERTILIZER_NAME;
		if (potion == PotionData.EMPTY_VIAL_META) return Names.EMPTY_VIAL_NAME;
		if (potion == PotionData.POTION_META) return Names.POTION_NAME;
		if (potion == PotionData.SPEED_META) return Names.SPEED_NAME;
		if (potion == PotionData.DIGGING_META) return Names.DIGGING_NAME;
		if (potion == PotionData.STRENGTH_META) return Names.STRENGTH_NAME;
		if (potion == PotionData.HEALING_META) return Names.HEALING_NAME;
		if (potion == PotionData.BOUNDING_META) return Names.BOUNDING_NAME;
		if (potion == PotionData.REGENERATION_META) return Names.REGENERATION_NAME;
		if (potion == PotionData.RESISTANCE_META) return Names.RESISTANCE_NAME;
		if (potion == PotionData.FIRE_WARDING_META) return Names.FIRE_WARDING_NAME;
		if (potion == PotionData.BREATHING_META) return Names.BREATHING_NAME;
		if (potion == PotionData.INVISIBILITY_META) return Names.INVISIBILITY_NAME;
		if (potion == PotionData.INFRAVISION_META) return Names.INFRAVISION_NAME;
		if (potion == PotionData.PROTECTION_META) return Names.PROTECTION_NAME;
		if (potion == PotionData.POTENCE_META) return Names.POTENCE_NAME;
		if (potion == PotionData.CELERITY_META) return Names.CELERITY_NAME;
		if (potion == PotionData.PANACEA_META) return Names.PANACEA_NAME;
		if (potion == PotionData.WATER_META) return Names.WATER_NAME;
	...
	public void registerIcons(IconRegister iconRegister) {
		iconBase = iconRegister.registerIcon(Reference.MOD_ID.toLowerCase() + ":" + Names.CONDENSED_POTION_NAME);
	...
	public Icon getIcon(ItemStack itemStack, int renderPass) {
		if (isEmptyVial(itemStack)) return iconBase;
		if (isPanacea(itemStack) || isPotion(itemStack) || isBasePotion(itemStack) || isJustWater(itemStack)) {
			if (renderPass == 1) return iconBaseOverlay;
			else return iconBase;
		} else {
	...
	public EnumAction getItemUseAction(ItemStack ist) {
		if (isPotion(ist)) return EnumAction.drink;
