...
public class AlkahestHandler implements ICraftingHandler {
	...
	public void onCrafting(EntityPlayer player, ItemStack item, IInventory craftMatrix) {
		if (item == null) return;
		if (item.getItem() != XRItems.alkahest) return;
		int totalPoints = 0;
		...
		System.out.println("At the time of execution, total value sits at " + totalPoints);
		while (totalPoints >= 2304) {
			if (!player.inventory.addItemStackToInventory(new ItemStack(Block.blockLapis, 1))) {
				player.dropPlayerItem(new ItemStack(Block.blockLapis, 1));
			}
			totalPoints -= 2304;
		}
		while (totalPoints >= 256) {
			if (!player.inventory.addItemStackToInventory(new ItemStack(Item.dyePowder, 1, 4))) {
				player.dropPlayerItem(new ItemStack(Item.dyePowder, 1, 4));
			}
			totalPoints -= 256;
		}
		while (totalPoints >= 64) {
			if (!player.inventory.addItemStackToInventory(new ItemStack(Item.redstone, 1))) {
