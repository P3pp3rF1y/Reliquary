...
public class EntityGlowingWater extends EntityThrowable {
    ...
    public EntityGlowingWater(World par1World, double par2, double par4,
    ...
    protected void onImpact(MovingObjectPosition mop) {
        if (!worldObj.isRemote) {
            ...
            while (i.hasNext()) {
                ...
                if (isUndead(e) && this.getThrower() != null
                        && this.getThrower() instanceof EntityPlayer) {
                    e.attackEntityFrom(
                            DamageSource.causePlayerDamage((EntityPlayer) this
                                    .getThrower()), 18 + rand.nextInt(17));
                }
            }
    ...
    private void spawnParticles() {
        for (int particles = 0; particles < 40; particles++) {
            worldObj.spawnParticle("mobSpellAmbient",
                    posX + rand.nextGaussian() * 4, posY + rand.nextGaussian()
                            * 4, posZ + rand.nextGaussian() * 4, 1.0F, 1.0F,
                    0.0F);
        }
    }
