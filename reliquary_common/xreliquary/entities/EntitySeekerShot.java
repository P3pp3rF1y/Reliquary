...
public class EntitySeekerShot extends Entity implements IProjectile {
    ...
    public EntitySeekerShot(World par1World, double par2, double par4,
            double par6) {
        super(par1World);
        this.setSize(0.5F, 0.5F);
    ...
    public EntitySeekerShot(World par1World, EntityPlayer par2EntityPlayer) {
        ...
        this.setLocationAndAngles(par2EntityPlayer.posX, par2EntityPlayer.posY
                + par2EntityPlayer.getEyeHeight(), par2EntityPlayer.posZ,
    ...
    public void onUpdate() {
        ...
        if (shootingEntity == null) {
            List players = worldObj.getEntitiesWithinAABB(EntityPlayer.class,
                    AxisAlignedBB.getBoundingBox(posX - 1, posY - 1, posZ - 1,
        ...
        if (ticksInAir > 3) {
            ...
            List targetsList = worldObj.getEntitiesWithinAABBExcludingEntity(
                    this, AxisAlignedBB.getBoundingBox(posX - 5, posY - 5,
                            posZ - 5, posX + 5, posY + 5, posZ + 5));
            Iterator iTarget = targetsList.iterator();
            double closestDistance = Double.MAX_VALUE;
            while (iTarget.hasNext()) {
                Entity currentTarget = (Entity) iTarget.next();
                if (!(currentTarget instanceof EntityLiving)) {
                    continue;
                }
                if (currentTarget == shootingEntity) {
                    continue;
                }
            ...
            if (closestTarget != null) {
                ...
                motionY = Math.sqrt(Math.abs(closestTarget.posY
                        + closestTarget.height / 1.75 - posY)) / 6;
                motionY *= closestTarget.posY + closestTarget.height / 1.75
                        - posY < 0 ? -1 : 1;
        ...
        if (var16 > 0) {
            Block.blocksList[var16].setBlockBoundsBasedOnState(worldObj, xTile,
                    yTile, zTile);
            ...
            if (var2 != null
                    && var2.isVecInside(worldObj.getWorldVec3Pool()
        ...
        if (inGround) {
            ...
            if (ticksInAir > 1 && ticksInAir < 3) {
                worldObj.spawnParticle("flame", posX + smallGauss(0.1D), posY
                        + smallGauss(0.1D), posZ + smallGauss(0.1D), 0D, 0D, 0D);
            ...
            Vec3 var17 = worldObj.getWorldVec3Pool().getVecFromPool(posX, posY,
                    posZ);
            Vec3 var3 = worldObj.getWorldVec3Pool().getVecFromPool(
                    posX + motionX, posY + motionY, posZ + motionZ);
            MovingObjectPosition var4 = worldObj.rayTraceBlocks_do_do(var17,
                    var3, false, true);
            var17 = worldObj.getWorldVec3Pool()
                    .getVecFromPool(posX, posY, posZ);
    ...
    private void doFlightParticles() {
        if (ticksInAir % 3 == 0) {
            worldObj.spawnParticle("mobSpellAmbient", posX, posY, posZ, 0.0F,
                    gaussian(1.0F), gaussian(1.0F));
        }
    }

    private void doFiringParticles() {
        worldObj.spawnParticle("mobSpellAmbient", posX + smallGauss(0.1D), posY
                + smallGauss(0.1D), posZ + smallGauss(0.1D), 0.5D, 0.5D, 0.5D);
        worldObj.spawnParticle("flame", posX, posY, posZ, gaussian(motionX),
                gaussian(motionY), gaussian(motionZ));
    }
    ...
    private void onImpact(Entity mop) {
        if (mop == shootingEntity && ticksInAir > 3) {
            shootingEntity.attackEntityFrom(
                    DamageSource.causePlayerDamage(shootingEntity), 1);
            this.setDead();
        } else {
            doDamage(12 + d6(), mop);
        }
    ...
    private void spawnHitParticles(int i) {
        for (int particles = 0; particles < i; particles++) {
            worldObj.spawnParticle("mobSpellAmbient", posX + smallGauss(0.1D),
                    posY + smallGauss(0.1D), posZ + smallGauss(0.1D), 0.1F,
                    1.0F, 1.0F);
        }
    }
            ...
            for (int particles = 0; particles < 4; particles++) {
                ...
                case 0:
                    worldObj.spawnParticle("smoke", posX, posY, posZ,
                            gaussian(0.1D), -gaussian(0.1D), gaussian(0.1D));
                    break;
                case 1:
                    worldObj.spawnParticle("smoke", posX, posY, posZ,
                            gaussian(0.1D), gaussian(0.1D), gaussian(0.1D));
